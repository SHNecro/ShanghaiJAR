...
using Avalonia.Controls;
 ...
 // using Avalonia.Input;using DXJoystick = SlimDX.DirectInput.Joystick;
...
namespace KeyConfigLinux.Controls
{
    /// <summary>
    ...
    {
	    public static readonly StyledProperty<string> LabelTextProperty = AvaloniaProperty.Register<EntryField, string>(nameof(LabelText));
	    public static readonly StyledProperty<string> EntryTextProperty = AvaloniaProperty.Register<EntryField, string>(nameof(EntryText));
	    public static readonly StyledProperty<string> DisplayTextProperty = AvaloniaProperty.Register<EntryField, string>(nameof(DisplayText));
        ...
        static EntryField()
		{
            ...
            EntryTextProperty.Changed.Subscribe(args =>
            {
	            if (args.Sender is EntryField sender && int.TryParse(sender.EntryText, out var entryCode))
	            {
		            sender.DisplayText = sender.IsKeyboardEntry
			            ? entryCode.FromSHKeyCode().ToString()
			            : entryCode.FromSHButtonCode().ToString();
	            }
            });
		...
		public EntryField()
        {
			...
			(Application.Current.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime).Exit +=
				(obj, args) =>
				{
					EntryField.InputComplete -= this.EntryField_OnInputComplete;
					EntryField.OnTKKeyPress -= this.EntryField_OnTKKeyPress;
