...
namespace NSEnemy
{
    internal class OrinMook2 : EnemyBase
    {
        private OrinMook2.MOTION motion = OrinMook2.MOTION.neutral;
        ...
        public OrinMook2(IAudioEngine s, SceneBattle p, int pX, int pY, byte n, Panel.COLOR u, byte v)
          : base(s, p, pX, pY, n, u, v)
        {
            this.wantedPosition.X = -24;
            for (int index = 0; index < this.dropchips.Length; ++index)
                this.dropchips[index] = new ChipFolder(this.sound);
            this.picturename = "OrinMook1";
            ...
            this.animationpoint.X = this.AnimeNeutral_x(0).X;
            this.animationpoint.Y = this.AnimeNeutral_y(0).X;
        ...
        {
            this.neutlal = this.motion == OrinMook2.MOTION.neutral;
            if (this.moveflame)
            {
                this.positionre = this.position;
                switch (this.motion)
                {
                    case OrinMook2.MOTION.neutral:
                        if (this.moveflame)
                        {
                            this.animationpoint.X = this.AnimeNeutral_x(this.frame).X;
                            this.animationpoint.Y = this.AnimeNeutral_y(this.frame).X;
                            ...
                            if (this.roopneutral >= 2 && this.parent.nowscene != SceneBattle.BATTLESCENE.end)
                            {
                                ...
                                if (this.EnemySearch(this.position.Y) && !this.badstatus[4])
                                {
                                    this.speed /= 2;
                                    ++this.attackroop;
                                    this.motion = OrinMook2.MOTION.attack;
                                    this.counterTiming = true;
                                }
                                else
                                {
                                    this.Xmove = false;
                                    this.motion = OrinMook2.MOTION.move;
                                }
                                /*
                    ...
                    case OrinMook2.MOTION.move:
                        ...
                        this.motion = OrinMook2.MOTION.neutral;
                        break;
                    case OrinMook2.MOTION.attack:
                        ...
                        if (this.frame == 4 + this.Attackinterval())
                        {
                            ...
                            yuyuFireball fireball1 = new yuyuFireball(this.sound, this.parent, this.position.X + this.UnionRebirth, this.position.Y, this.union, this.Power, 2, new Vector2(this.positionDirect.X, this.positionDirect.Y + 0f), ChipBase.ELEMENT.normal, butMov)
                            {
                        ...
                        if (this.frame >= 12 + this.Attackinterval())
                        {
                            this.plusX = 0;
                            this.speed = this.nspeed;
                            this.motion = OrinMook2.MOTION.neutral;
            ...
            if (this.motion == OrinMook2.MOTION.attack && (this.frame >= 3 + this.Attackinterval() && this.frame <= 6 + this.Attackinterval()))
            {
        ...
        private Point AnimeNeutral(int waitflame)
        {
            return this.Return(
                ...
                new int[7] { 13, 14, 15, 16, 17, 18, 19 }, 0, waitflame);
        }*/

        private Point AnimeNeutral_x(int waitflame)
        {
            return this.Return(
                new int[7] { 0, 1, 2, 3, 4, 5, 6 },
                new int[7] { 13, 14, 15, 16, 17, 0, 1 }, 
                 0, waitflame);
        }

        private Point AnimeNeutral_y(int waitflame)
        {
            return this.Return(
                new int[7] { 0, 1, 2, 3, 4, 5, 6 },
                new int[7] { 0, 0, 0, 0, 0, 1, 1 },
                 0, waitflame);
        }
        /*
        private Point AnimeAttack(int waitflame)
        {
            return this.ReturnKai(
                new int[8] { 0, 1, 1, 1, this.Attackinterval(), 1, 1, 30 }, 
