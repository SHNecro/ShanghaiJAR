...
namespace NSObject
{
    internal class BigSuzuran : ObjectBase
    {
        ...
        protected override void AttackMake(int power, int slideX = 0, int slideY = 0, bool break_ = false)
        {
            if (!this.effecting)
                return;
            var enemyHit = new EnemyHit(this.sound, this.parent, this.position.X + slideX, this.position.Y + slideY, this.union, power, this.element, this)
            {
                breaking = break_
            };

            enemyHit.BadStatusSet(CharacterBase.BADSTATUS.poison, 150 * 8);
            enemyHit.BadStatusSet(CharacterBase.BADSTATUS.blind, 300);
