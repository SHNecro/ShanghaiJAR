...
namespace NSChip
{
    internal class SamonReyCanon : ChipBase
  {
    private const int start = 1;
    private const int speed = 2;

    public SamonReyCanon(IAudioEngine s)
      : base(s)
    {
      this.number = 202;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.SamonReyCanonName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.SamonReyCanonDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      else if (character.waittime == 15)
      {
        ...
        battle.enemys.Add(new ReyCanon(this.sound, battle, character.positionold.X, character.positionold.Y, (byte)battle.enemys.Count, Panel.COLOR.red, 2));
      }
      else
      {
      ...
      bool noicon)
    {
      if (!noicon)
      {
        ...
        if (select)
          num = 1;
        this._rect = new Rectangle(208, num * 16, 16, 16);
    ...
    {
      this._rect = new Rectangle(player.animationpoint.X * player.Wide, 272, player.Wide, player.Height);
