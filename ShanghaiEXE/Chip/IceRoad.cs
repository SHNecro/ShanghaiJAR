...
namespace NSChip
{
    internal class IceRoad : ChipBase
  {
    private const int speed = 2;

    public IceRoad(IAudioEngine s)
      : base(s)
    {
      this.number = 162;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.IceRoadName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.IceRoadDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          ...
          {
            battle.panel[pX, y].state = Panel.PANEL._ice;
