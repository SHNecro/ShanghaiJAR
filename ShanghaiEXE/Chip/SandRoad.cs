...
namespace NSChip
{
    internal class SandRoad : ChipBase
  {
    private const int speed = 2;

    public SandRoad(IAudioEngine s)
      : base(s)
    {
      this.number = 168;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.SandRoadName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.SandRoadDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          ...
          {
            battle.panel[pX, y].state = Panel.PANEL._sand;
