...
namespace NSChip
{
    internal class GigaTomahawk : ChipBase
  {
    private const int start = 1;
    private const int speed = 2;

    public GigaTomahawk(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-3, 0);
      this.number = 105;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.GigaTomahawkName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.GigaTomahawkDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      character.parent.attacks.Add(this.Paralyze(new NSAttack.Tomahawk(this.sound, character.parent, character.position.X + this.UnionRebirth(character.union), character.position.Y, character.union, this.Power(character), 1, 2, this.element)));
    }
