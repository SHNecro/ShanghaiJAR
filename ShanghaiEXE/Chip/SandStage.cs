...
namespace NSChip
{
    internal class SandStage : ChipBase
  {
    private const int speed = 2;

    public SandStage(IAudioEngine s)
      : base(s)
    {
      this.number = 169;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.SandStageName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.SandStageDesc");
      ...
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          for (int pY = 0; pY < battle.panel.GetLength(1); ++pY)
          {
            ...
            {
              battle.panel[pX, pY].state = Panel.PANEL._sand;
