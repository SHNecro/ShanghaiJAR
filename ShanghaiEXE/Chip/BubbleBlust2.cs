...
namespace NSChip
{
  internal class BubbleBlust2 : ChipBase
  {
    private const int shotend = 10;
    private const int shotinterval = 4;

    public BubbleBlust2(MyAudio s)
      : base(s)
    {
      this.rockOnPoint = new Point(-3, 0);
      this.number = 54;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.BubbleBlust2Name");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.BubbleBlust2Desc");
    ...
    {
      character.animationpoint = BubbleBlust2.Animation(character.waittime);
      ...
      character.parent.attacks.Add(this.Paralyze(new Vulcan(this.sound, character.parent, character.position.X + this.UnionRebirth(character.union), character.position.Y, character.union, this.Power(character), Vulcan.SHOT.Bubble, ChipBase.ELEMENT.aqua, false)));
    }
    ...
    {
      this._rect = new Rectangle(4 * character.Wide, 2 * character.Height, character.Wide, character.Height);
