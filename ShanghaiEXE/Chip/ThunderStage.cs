...
namespace NSChip
{
    internal class ThunderStage : ChipBase
  {
    private const int speed = 2;

    public ThunderStage(IAudioEngine s)
      : base(s)
    {
      this.number = 167;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.ThunderStageName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.ThunderStageDesc");
      ...
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          for (int pY = 0; pY < battle.panel.GetLength(1); ++pY)
          {
            ...
            {
              battle.panel[pX, pY].state = Panel.PANEL._thunder;
