...
namespace NSChip
{
    internal class IceStage : ChipBase
  {
    private const int speed = 2;

    public IceStage(IAudioEngine s)
      : base(s)
    {
      this.number = 163;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.IceStageName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.IceStageDesc");
      ...
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          for (int pY = 0; pY < battle.panel.GetLength(1); ++pY)
          {
            ...
            {
              battle.panel[pX, pY].state = Panel.PANEL._ice;
