...
namespace NSChip
{
    internal class MagicBombX : ChipBase
  {
    private const int start = 3;
    private const int speed = 3;

    public MagicBombX(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-3, 0);
      this.number = 374;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.MagicBombXName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.MagicBombXDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      if (character.waittime == 6)
      {
        ...
        battle.attacks.Add(this.Paralyze(new ClossBomb(this.sound, character.parent, character.position.X, character.position.Y, character.union, this.Power(character), 1, new Vector2(character.positionDirect.X, character.positionDirect.Y - 16f), end, 40, ClossBomb.TYPE.closs, true, ClossBomb.TYPE.single, false, false)));
      }
      ...
      bool noicon)
    {
      if (!noicon)
      {
        ...
        if (select)
          num4 = 1;
        this._rect = new Rectangle(400, 80 + num4 * 96, 16, 16);
      ...
      bool printstatus)
    {
      if (printgraphics)
      {
        this._rect = new Rectangle(1568, 0, 56, 48);
