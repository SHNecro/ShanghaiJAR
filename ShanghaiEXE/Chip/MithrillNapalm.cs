...
namespace NSChip
{
    internal class MithrillNapalm : ChipBase
  {
    private const int start = 3;
    private const int speed = 3;

    public MithrillNapalm(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-3, 0);
      this.swordtype = false;
      this.number = 83;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.MithrillNapalmName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.MithrillNapalmDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      if (character.waittime == 6)
      {
        ...
        battle.attacks.Add(this.Paralyze(new NapalmBomb(this.sound, character.parent, character.position.X, character.position.Y, character.union, this.Power(character), 1, new Vector2(character.positionDirect.X, character.positionDirect.Y - 16f), end, 40, NapalmBomb.TYPE.big, 1800)));
      }
