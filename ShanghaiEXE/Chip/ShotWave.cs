...
namespace NSChip
{
    internal class ShotWave : ChipBase
  {
    private const int start = 1;
    private const int speed = 2;

    public ShotWave(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-1, 0);
      this.number = 17;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.ShotWaveName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.ShotWaveDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      character.parent.attacks.Add(this.Paralyze(new WaveAttsck(this.sound, character.parent, character.position.X + this.UnionRebirth(character.union), character.position.Y, character.union, this.Power(character), 6, 0, this.element)));
    }
