...
namespace NSChip
{
    internal class GrassRoad : ChipBase
  {
    private const int speed = 2;

    public GrassRoad(IAudioEngine s)
      : base(s)
    {
      this.number = 164;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.GrassRoadName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.GrassRoadDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          ...
          {
            battle.panel[pX, y].state = Panel.PANEL._grass;
