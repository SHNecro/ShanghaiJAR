...
namespace NSChip
{
    internal class ColdAir3 : ChipBase
  {
    private bool open;
    private const int shotend = 10;

    public ColdAir3(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-3, 0);
      this.number = 26;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.ColdAir3Name");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.ColdAir3Desc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      ...
      character.parent.attacks.Add(this.Paralyze(new PushTornado(this.sound, character.parent, character.position.X, character.position.Y, character.union, this.Power(character), ChipBase.ELEMENT.aqua, 8, 3, false)));
    }
    ...
    public override void Render(IRenderer dg, CharacterBase character)
    {
      if (character.waittime < 5)
        return;
      this._rect = new Rectangle(7 * character.Wide, 4 * character.Height, character.Wide, character.Height);
