...
namespace NSChip
{
    internal class GraviBall2 : ChipBase
  {
    private const int speed = 4;
    private const int shotend = 16;

    public GraviBall2(IAudioEngine s)
      : base(s)
    {
      this.rockOnPoint = new Point(-1, 0);
      this.number = 41;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.GraviBall2Name");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.GraviBall2Desc");
    ...
    public override void Render(IRenderer dg, CharacterBase character)
    {
      if (character.waittime < 5)
        return;
      this._rect = new Rectangle(2 * character.Wide, character.Height, character.Wide, character.Height);
