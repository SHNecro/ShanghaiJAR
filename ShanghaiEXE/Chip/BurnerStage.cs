...
namespace NSChip
{
    internal class BurnerStage : ChipBase
  {
    private const int speed = 2;

    public BurnerStage(IAudioEngine s)
      : base(s)
    {
      this.number = 161;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.BurnerStageName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.BurnerStageDesc");
      ...
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          for (int pY = 0; pY < battle.panel.GetLength(1); ++pY)
          {
            ...
            {
              battle.panel[pX, pY].state = Panel.PANEL._burner;
