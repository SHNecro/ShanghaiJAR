...
namespace NSChip
{
    internal class ThunderRoad : ChipBase
  {
    private const int speed = 2;

    public ThunderRoad(IAudioEngine s)
      : base(s)
    {
      this.number = 166;
      this.name = NSGame.ShanghaiEXE.Translate("Chip.ThunderRoadName");
      ...
      var information = NSGame.ShanghaiEXE.Translate("Chip.ThunderRoadDesc");
    ...
    public override void Action(CharacterBase character, SceneBattle battle)
    {
      if (character.waittime == 1)
      {
        ...
        for (int pX = 0; pX < battle.panel.GetLength(0); ++pX)
        {
          ...
          {
            battle.panel[pX, y].state = Panel.PANEL._thunder;
