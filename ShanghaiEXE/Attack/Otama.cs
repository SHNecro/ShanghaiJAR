...
namespace NSAttack
{
    internal class Otama : AttackBase
    {
        private bool movestart = false;
        private Otama.Mothion mothion;
                            ...
                            foreach (CharacterBase characterBase in this.parent.AllHitter())
                            {
                                if (characterBase.position.X == this.position.X && characterBase.union == this.UnionEnemy && !characterBase.nohit)
                                {
                                    if (characterBase.position.Y > this.position.Y)
                                        this.mothion = Otama.Mothion.down;
                                    else if (characterBase.position.Y < this.position.Y)
                                        this.mothion = Otama.Mothion.up;
                                }
                            }
                            break;
                        }
                        break;
                    case Otama.Mothion.up:
                        this.positionDirect.Y -= movespeed;
                        break;
                    case Otama.Mothion.down:
                        this.positionDirect.Y += movespeed;
                        break;
                }
                if (this.mothion != Otama.Mothion.go) { }
        ...
        public override void Render(IRenderer dg)
        {
            ...
            this._rect = new Rectangle(this.animationpoint.X * 24, y, 24, 24);
